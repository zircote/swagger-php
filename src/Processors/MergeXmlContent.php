    ...
    public function __invoke(Analysis $analysis)
    {
        ...
        foreach ($annotations as $xmlContent) {
            ...
            if (!($parent instanceof Response) && !($parent instanceof RequestBody) && !($parent instanceof Parameter)) {
                if ($parent) {
                    $xmlContent->_context->logger->notice('Unexpected ' . $xmlContent->identity() . ' in ' . $parent->identity() . ' in ' . $parent->_context);
                } else {
                    $xmlContent->_context->logger->notice('Unexpected ' . $xmlContent->identity() . ' must be nested');
                }
                continue;
            }
