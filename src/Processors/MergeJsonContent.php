    ...
    public function __invoke(Analysis $analysis)
    {
        ...
        foreach ($annotations as $jsonContent) {
            ...
            if (!($parent instanceof Response) && !($parent instanceof RequestBody) && !($parent instanceof Parameter)) {
                if ($parent) {
                    $jsonContent->_context->logger->notice('Unexpected ' . $jsonContent->identity() . ' in ' . $parent->identity() . ' in ' . $parent->_context);
                } else {
                    $jsonContent->_context->logger->notice('Unexpected ' . $jsonContent->identity() . ' must be nested');
                }
                continue;
            }
