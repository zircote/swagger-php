        ...
        if ($analysis->openapi->paths !== Generator::UNDEFINED) {
            foreach ($analysis->openapi->paths as $annotation) {
                if (empty($annotation->path)) {
                    $annotation->_context->logger->notice($annotation->identity() . ' is missing required property "path" in ' . $annotation->_context);
        ...
        foreach ($operations as $operation) {
            if ($operation->path) {
                ...
                if ($paths[$operation->path]->merge([$operation])) {
                    $operation->_context->logger->notice('Unable to merge ' . $operation->identity() . ' in ' . $operation->_context);
                }
            }
        }
        if (count($paths)) {
