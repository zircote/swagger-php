...
abstract class AbstractAnnotation implements \JsonSerializable
{
            ...
            if (property_exists($this, $property)) {
                ...
                foreach ($value as $annotation) {
                    if (is_object($annotation) && $annotation instanceof AbstractAnnotation) {
                        ...
                        $this->_context->logger->notice('Unexpected field in ' . $this->identity() . ' in ' . $this->_context);
                    }
                }
                ...
                $this->_context->logger->notice('Unexpected parameter in ' . $this->identity());
            }
        }
    }
    ...
    public function __get($property)
    {
        ...
        $this->_context->logger->notice('Property "' . $property . '" doesn\'t exist in a ' . $this->identity() . ', existing properties: "' . implode('", "', array_keys($properties)) . '" in ' . $this->_context);
    }
    ...
    public function __set($property, $value)
    {
        ...
        $this->_context->logger->notice('Unexpected field "' . $property . '" for ' . $this->identity() . ', expecting "' . implode('", "', array_keys($fields)) . '" in ' . $this->_context);
        $this->$property = $value;
    }
    ...
    public function mergeProperties($object): void
    {
        ...
        foreach ($object as $property => $value) {
            ...
            if ($currentValues[$property] !== $value) { // New value is not the same?
                ...
                $this->_context->logger->warning('Multiple definitions for ' . $identity . '->' . $property . "\n     Using: " . $context1 . "\n  Skipping: " . $context2);
            }
        }
    }
    ...
    public function validate(array $parents = [], array $skip = [], string $ref = ''): bool
    {
        ...
        foreach ($this->_unmerged as $annotation) {
            if (!is_object($annotation)) {
                $this->_context->logger->notice('Unexpected type: "' . gettype($annotation) . '" in ' . $this->identity() . '->_unmerged, expecting a Annotation object');
                break;
            }
        ...
        foreach (static::$_nested as $annotationClass => $nested) {
            ...
            foreach ($this->$property as $key => $item) {
                if (is_array($item) && is_numeric($key) === false) {
                    ...
                    $this->_context->logger->warning('Multiple ' . $item->_identity([]) . ' with the same ' . $keyField . '="' . $item->$keyField . "\":\n  " . $item->_context . "\n  " . $keys[$item->$keyField]->_context);
                } else {
        ...
        if (property_exists($this, 'ref') && $this->ref !== Generator::UNDEFINED && $this->ref !== null) {
            if (substr($this->ref, 0, 2) === '#/' && count($parents) > 0 && $parents[0] instanceof OpenApi) {
                ...
                try {
                    ...
                    $this->_context->logger->notice($e->getMessage() . ' for ' . $this->identity() . ' in ' . $this->_context);
                }
            }
        } else {
            ...
            foreach (static::$_required as $property) {
                if ($this->$property === Generator::UNDEFINED) {
                    ...
                    $this->_context->logger->notice($message);
                }
            }
        }
        ...
        foreach (static::$_types as $property => $type) {
            ...
            if (is_string($type)) {
                if ($this->validateType($type, $value) === false) {
                    $valid = false;
                    $this->_context->logger->notice($this->identity() . '->' . $property . ' is a "' . gettype($value) . '", expecting a "' . $type . '" in ' . $this->_context);
                }
            } elseif (is_array($type)) { // enum?
                if (in_array($value, $type) === false) {
                    $this->_context->logger->notice($this->identity() . '->' . $property . ' "' . $value . '" is invalid, expecting "' . implode('", "', $type) . '" in ' . $this->_context);
                }
            } else {
