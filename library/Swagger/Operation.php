...
class Operation extends AbstractEntity
{
     ...
     * @return \Swagger\Operation
     */
    protected function _getInternalResponseType()
    {
        if (preg_match(
            self::PATTERN_INTERNAL_RESP, $this->_docComment, $matches
        )
        ) {
            $this->results['responseTypeInternal'] = $matches[1];
        }
        return $this;
    }
    ...
    {
        if (preg_match_all(
            self::PATTERN_OPERATION, $this->_docComment, $matches
        )
        ) {
            foreach ($matches[1] as $match) {
                $this->results = array_merge_recursive(
                    $this->results, $this->_parseParts($match)
                );
            }
    ...
    {
        if (preg_match_all(
            self::PATTERN_APIERROR, $this->_docComment, $matches
        )
        ) {
            foreach ($matches[1] as $match) {
                array_push(
                    $this->results['errorResponses'], $this->_parseParts($match)
                );
            }
        }
        return $this;
    ...
    protected function _getParam()
    {
        if (preg_match_all(self::PATTERN_APIPARAM, $this->_docComment, $matches)) {
            foreach ($matches[1] as $match) {
                ...
                array_push(
                    $this->results['parameters'], $apiOperation->results
                );
            }
        }
        return $this;
